...
if (isset($_POST['sub'])) {
    ...
    $query = "INSERT INTO orders(item_name, price, name, phone, address, ord_no) 
              VALUES ('$item_name', '$price', '$name', '$phone', '$address', '$ordno')";

    if (mysqli_query($connection, $query)) {
        header("Location: order_sent.php?" . $queryParameters);
    }else {
...
<body>
    <!-- Header -->
    <nav class="navbar navbar-expand-lg navbar-light shadow-lg" style="background-color: #ced4da;">
    ...
    $stmt = $connection -> prepare ("SELECT * FROM item WHERE img = ?");
    $stmt -> bind_param ("s", $i);
    $stmt -> execute();

    $result = $stmt -> get_result();

    $mat = $result -> fetch_array();
    $stmt -> Close();
            ...
            <div class="row">
                <div class="col-lg-5 mt-3">
                    <div class="card mb-3 shadow-lg">
                        <img class="card-img img-fluid" src="admin/assets/image/<?php echo $i;?>" id="product-detail">
                    </div>
                </div>
                    ...
                    <div class="card shadow-lg">
                        <div class="card-body">
                            ...
                            <div class="mb-2 col-lg-12">
                                <label for="name" class="form-label fw-bold text-success">Item Name</label>
                                <input type="text" class="form-control shadow bg-light" name="item_name" value="<?php echo $mat['item_name']?>" readonly>
    
                            </div>
                            <div class="mb-2 col-lg-12">
                                <label for="name" class="form-label fw-bold text-success">Price</label>
                                <input type="text" class="form-control shadow bg-light" name="price" value="<?php echo $mat['price']?>" readonly>
    
                            </div>

                            <div class="mb-2 col-lg-12">
                                <label for="name" class="form-label fw-bold text-success">Name</label>
                                <input type="text" class="form-control shadow bg-light" name="name" required>
    
                            </div>
                            <div class="mb-2 col-lg-12">
                                <label for="phone" class="form-label fw-bold text-success">Phone</label>
                                <input type="text" class="form-control shadow bg-light" name="phone" required>
    
                            </div>
                            <div class="mb-4 col-lg-12">
                                <label for="address" class="form-label fw-bold text-success">Address</label>
                                <input type="text" class="form-control shadow bg-light" name="address" required>
                            </div>
                                
                                <div class="row pb-3">
                                    <div class="col-lg-12 d-grid justify-content-lg-center">
                                        <button type="submit" class="btn btn-outline-success btn-md shadow" name="sub">Send Order</button>
                                    </div>
